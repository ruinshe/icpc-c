...
struct lct_t {
  ...
  void rotate(node_t *x, int f) {
    ...
    {
      if (y == z->c[0]) z->c[0] = x;
      else if (y == z->c[1]) z->c[1] = x;
    }
    y->c[f ^ 1] = x->c[f];
    if (y->c[f ^ 1] != nil) y->c[f ^ 1]->p = y;
    x->c[f] = y;
    update(y);
    update(x);
  }
  ...
  void splay(node_t *x) {
    ...
    while (!isroot(x)) {
      node_t *y = x->p;
      if (isroot(y)) rotate(x, y->c[0] == x);
      else {
        int t = (y == y->p->c[0]);
        if (x == y->c[t]) rotate(x, t ^ 1), rotate(x, t);
  ...
  node_t *getRoot(node_t *x) {
    return head(expose(x));
  }
  ...
  void cut(node_t *x) {
    splay(x);
    if (x->c[0] != nil) x->c[0]->p = x->p, x->p = x->c[0] = nil;
    else x->p = nil;
  }
